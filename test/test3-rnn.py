...
#     url="http://www.google.com/finance/historical?q=%s&ei=u-lHWfGPNNWIsgHHqIqICw&output=csv" % stock_name 
    ...
    stocks = pd.read_csv('round1_raw2.csv', header=0, names=col_names) 
...
def load_data(stock, seq_len):
    ...
    y_test = result[int(row):, -1, -1]   #result[int(row):-10, 0, -1] # do not include last 10 rows as thats true_test, orig: y_test = result[int(row):, -1][:,-1]
...
def main():
    ...
    model.fit(
        ...
        validation_split=0.3, #originally 0.1
        verbose=0,
        callbacks=[out_batch])
